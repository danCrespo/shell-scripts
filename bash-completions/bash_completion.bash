...
# shellcheck disable=SC2162,SC2034,SC2015,SC2128,SC2155,SC2016,SC1090
...
quote() {
    echo -e "\'${1//\'/\'\\\'\'}\'" #'# Help vim syntax highlighting
}
...
_upvars() {
    ...
    while (($#)); do
        ...
        -a*)
            # Error checking
            [[ ${1#-a} ]] || {
                echo "bash: ${FUNCNAME[0]}: \`$1': missing" \
                    "number specifier" 1>&2
                return 1
            }
            ...
            [[ "$2" ]] && unset -v "$2" && eval "$2"="\(\"\${@:3:${1#-a}}\"\)" &&
                shift $((${1#-a} + 2)) || {
                echo "bash: ${FUNCNAME[0]}:" \
                    "\`$1${2+ }$2': missing argument(s)" 1>&2
                return 1
            }
            ;;
        -v)
...
list=("$(
    sed -ne '/^# START exclude/,/^# FINISH exclude/p' "$BASH_COMPLETION" |
